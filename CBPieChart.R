...
data <- read.csv("./Bionomia Stats/CBaldwinCSV.csv", header = TRUE)
...
pie(freq_table_grouped, main = "Countries Where Specimens Were Collected by Carole Baldwin", radius = 1, col = rainbow(length(freq_table_grouped)))

# Adjust the position of the legend
legend("topright", legend = names(freq_table_grouped), cex = 0.8, fill = rainbow(length(freq_table_grouped)), x.intersp = 0.5, y.intersp = .8, inset = c(-.21, 0.15))
